...
(function(root,factory){
  ...
  function resolve(years,values,year,policy='observed',metadata={}){
    ...
    if(index>=0 && annual && raw==null && policy==='earliest'){
      const first=years.map((y,i)=>({year:Number(y),value:values[i]})).filter(p=>positive(p.value)).sort((a,b)=>a.year-b.year)[0];
      if(first && Number(year)<first.year){value=first.value;sourceYear=first.year;substituted=true;}
    }
    const valid=index>=0 && positive(value);
