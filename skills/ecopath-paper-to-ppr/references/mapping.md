...
```bash
python scripts/prepare_mapping.py LME_047
```
...
```bash
python scripts/validate_mapping.py LME_047
```
...
`<model>.groups.csv` and `<model>.notes.md`, per `references/mapping-output-format.md`. One row per
...
`skills/ewe-species-to-group-mapper/examples/` holds three first-generation mappings — Arabian Sea, Bay of Bengal, East China
