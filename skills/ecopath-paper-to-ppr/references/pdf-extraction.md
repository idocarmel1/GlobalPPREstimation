...
`scripts/pdfgrid.py` wraps this: it groups words into rendered lines by vertical
overlap, merges words into cells by horizontal gap, and assigns cells to columns
by geometry. A misassignment then shows up as a visibly ragged grid or a diet
column that doesn't sum to 1, instead of as a plausible wrong number.

## The workflow

```bash
# 1. what pages exist, and are any rotated?
pdfinfo paper.pdf
pdftotext -bbox-layout -f 1 -l 1 paper.pdf - | head -5   # page width/height

# 2. look at the raw lines with coordinates before assuming anything
python scripts/pdfgrid.py paper.pdf 14 --raw

# 3. reconstruct the grid, anchoring columns on the header row
python scripts/pdfgrid.py paper.pdf 14 --merge-gap 0.4 --anchor-line 2 --sums

# 4. once it looks right, get machine-readable output
python scripts/pdfgrid.py paper.pdf 14 --merge-gap 0.4 --anchor-line 2 --tsv
```
...
has reversed x-coordinates, so column reconstruction on the page as stored
produces mirrored nonsense. Coordinates cannot be un-rotated reliably after the
fact — rotate the page physically first:

```bash
qpdf --rotate=+90:14-17 paper.pdf rotated.pdf   # then extract from rotated.pdf
```
...
```bash
pdftoppm -f 14 -l 14 -r 200 -png paper.pdf /tmp/page   # /tmp/page-14.png
```
...
| Symptom | Cause | Fix |
|---|---|---|
| Grid empty, no words | scanned page, no text layer | OCR is out of scope; transcribe the table by hand from a 300 dpi render and note it in the report |
