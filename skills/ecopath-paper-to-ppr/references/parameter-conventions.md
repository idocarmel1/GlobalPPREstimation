...
|---|---|---|---|
| Hab area (proportion) | fraction of the model area the group occupies | 1 | use 1 unless the paper states a habitat restriction |
...
- **Blank means steady state**, which is what most of this corpus assumes. It is
  the right answer far more often than not, and `validate.py` warns on a wholly
  blank file only so the report has to record that the paper was checked.
