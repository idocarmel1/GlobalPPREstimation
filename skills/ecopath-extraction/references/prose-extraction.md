...
python scripts/prose_sweep.py paper.pdf --family pointers

# the full sweep, restricted to sentences containing numbers
python scripts/prose_sweep.py paper.pdf --numbers-only

# chase one thing with surrounding context
python scripts/prose_sweep.py paper.pdf --family assimilation --context 2

# group-specific hunting
python scripts/prose_sweep.py paper.pdf --terms "phagotroph,ciliate,bacteria"
```
...
```bash
pdftoppm -f 12 -l 12 -r 300 -png paper.pdf /tmp/fig
```
