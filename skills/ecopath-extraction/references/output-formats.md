...
The path is printed on stdout so the validation steps can chain from it:

```bash
d=$(python scripts/write_outputs.py model.json --outdir .)
python scripts/validate.py "$d" && python scripts/massbalance_check.py "$d"
```
...
file is the steady-state case and the common one — `validate.py` warns on it so
that the report has to say the paper was checked, and where both columns are
filled it cross-checks them through B.
...
<either: the paper is steady-state and reports no BA (say where you looked), or
the values with their source — table and page, sentence and page, or the two
