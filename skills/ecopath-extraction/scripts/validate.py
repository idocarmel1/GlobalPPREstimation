...
def main() -> int:
    ...
    if (d / "Biomass_accumulation.csv").exists():
        ...
        if filled == 0:
            warn(
                "Biomass_accumulation.csv is entirely blank — fine for a "
                "steady-state model, but say in REPORT.md that the paper was "
                "checked for BA rather than leaving it looking unfilled"
            )
