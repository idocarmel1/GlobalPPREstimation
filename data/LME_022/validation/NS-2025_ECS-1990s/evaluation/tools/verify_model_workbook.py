...
PPR_CELL = re.compile(
    r'^=IFERROR\(IF\(ISNUMBER\(INDEX\(SPPR!\$([A-Z]+):\$([A-Z]+),'
    r'MATCH\(\$A(\d+),SPPR!\$A:\$A,0\),(\d+)\)\),\$C\3\*'
    r'INDEX\(SPPR!\$\1:\$\2,MATCH\(\$A\3,SPPR!\$A:\$A,0\),\4\),""\),""\)$')
...
def check(path: Path, unit: str) -> list:
    ...
    for j, method in enumerate(methods, 4):
        letter = openpyxl.utils.get_column_letter(j)
        block = f'{letter}{first}:{letter}{bytaxon.max_row}'
        expected = f'=IF(COUNT({block})=0,"",SUM({block}))'
        if bytaxon.cell(hdr_row + 1, j).value != expected:
            bad.append(f'{method}: total formula does not preserve an entirely missing method')
    ...
    # Check every year of the added scopes, independently summing the stored
    # taxon SPPR against catch. Missingness is checked separately from zero.
    for scope in ('inner', 'PP'):
        sn, pn = 'sppr_' + scope, 'PPR ' + scope
        if sn not in wbf.sheetnames or pn not in wbf.sheetnames:
            bad.append(f'missing scope sheets: {sn}, {pn}')
            continue
        sg, pg = grid(wbf[sn]), grid(wbf[pn])
        sh = sg[3]
        ph = next(r for r in pg if r and r[0] == 'method')
        pr = {r[0]: r for r in pg if r and r[0] in sh[4:]}
        for j, method in enumerate(sh[4:], 4):
            for y in years:
                terms = [catch[catch_row[r[0]]][catch_hdr.index(y)] * r[j]
                         for r in sg[4:] if r[0] in catch_row and isinstance(r[j], (int, float))
                         and catch[catch_row[r[0]]][catch_hdr.index(y)]]
                expected = round(sum(terms), 3) if terms else None
                actual = pr.get(method, [None] * len(ph))[ph.index(y)]
                if ((actual is None) != (expected is None) or
                    (actual is not None and abs(actual - expected) > max(.001, abs(expected) * 1e-9))):
                    bad.append(f'{scope} {method} {y}: aggregate {actual} != taxa {expected}')
