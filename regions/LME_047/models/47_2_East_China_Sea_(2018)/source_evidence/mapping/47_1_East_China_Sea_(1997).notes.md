# `47_1_East_China_Sea_(1997)` — mapping notes
...
`M1997` is the earlier of the paper's two mass-balance models, parameterised on a
1997–2000 marine living resources survey. Its companion, `M2018` (mapped separately as
`47_2_East_China_Sea_(2018)`), uses a 2018 autumn / 2019 spring joint survey.
...
the paper documents its membership fully.
...
## Which model does Supplementary Table S1 describe?

Its caption reads "…for the East China Sea (ECS) ecosystem model in **M2018**". This is the
1997 model, so that caveat matters and is the main thing a reviewer should push on.

The reasons for applying the same membership to both models:

1. §2.2 of the paper describes **one** set of 24 functional groups and applies it to both
   models: "There were 24 function groups in the ECS ecosystem, including … (Table S1). In
   addition, large yellow croakers, small yellow croakers, hairtails, and Bombay duck were
   treated as separate function groups." No second grouping is described anywhere.
2. Table 1 reports both models side by side under one list of group names, in the same
   order as the two diet matrices and as `groups_df`.
...
So the two mapping files are **identical row for row**, and deliberately so. What differs
between the models is what the guilds were made of in practice — the paper notes that
`Planktivores` came to be dominated by *Benthosema pterotum* by 2018 and that the M1997
keystone was `Planktivores/piscivores` dominated by *Decapterus maruadsi* — but that is a
statement about biomass, not about which species the modellers assigned where. A reviewer
who wanted to encode the 1997 composition differently would need a source the paper does
not provide.

The one place this leaks into the numbers is the weighting of composites. Weights left
blank are resolved by catch composition, which is the same for both files; a composite
switched to `model_biomass` would resolve differently per model, because M1997 and M2018
have very different group biomasses (`Planktivores/piscivores` 2.588 vs 1.294 t/km²,
`Planktivores` 0.710 vs 4.701). No composite in this mapping uses `model_biomass`.
...
the single largest decision in the mapping. The opposite choice is defensible — Chinese
trawl "trash fish" landings do contain a great deal of small fish — and it would move a
large share of the tonnage onto a group whose SPPR is very different.
...
*Nemipterus virgatus*)". The species' diet of shrimps, crabs and small fish agrees.
...
**8. Three inherited assignments were changed on diet grounds** beyond *Pennahia*:
