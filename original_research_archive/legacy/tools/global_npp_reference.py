...
def verify_source_files(root, downloads):
    ...
    for item in downloads:
        ...
        stat = path.stat()
        # Size and timestamps do not identify content: copies and fast rewrites
        # can preserve both. Revalidate bytes at every source-validation boundary.
...
def source_configuration_path(root, year):
    """Resolve canonical regional runs to their unique verified original source."""
    import csv
    root = Path(root).resolve()
    ...
    if canonical.exists():
        with canonical.open(encoding='utf-8-sig', newline='') as stream:
            references = {row['provenance'] for row in csv.DictReader(stream)
                          if int(row['year']) == int(year) and row.get('provenance')}
    directory = root / f'NPPExtraction/output/years/{year}'
    expansion = root / f'NPPExtraction/output/regional_expansion/years/{year}'
    originals = set()
    for reference in references:
        path = (root / reference).resolve()
        if not path.is_relative_to(directory.resolve()):
            if (not path.is_relative_to(expansion.resolve()) or
                    path.name != 'provenance.json' or not path.is_file()):
                raise ValueError(f'Canonical NPP source configuration is missing or outside its year: {year}')
            parent = json.loads(path.read_text(encoding='utf-8')).get('source_configuration')
            if not isinstance(parent, dict) or not parent.get('path') or not parent.get('sha256'):
                raise ValueError(f'Expanded NPP source configuration lacks verified parent lineage: {year}')
            path = (root / parent['path']).resolve()
            if not path.is_relative_to(directory.resolve()) or not path.is_file():
                raise ValueError(f'Expanded NPP source configuration parent is missing or outside its year: {year}')
            if sha256(path) != parent['sha256']:
                raise ValueError(f'Expanded NPP source configuration parent checksum mismatch: {year}')
        originals.add(path)
    if len(originals) > 1:
        raise ValueError(f'Conflicting canonical NPP source configurations for {year}')
    if originals:
        path = originals.pop()
        if not path.is_file():
            raise ValueError(f'Canonical NPP source configuration is missing or outside its year: {year}')
        return path
    matches = sorted(directory.glob('*/provenance.json'))
    if len(matches) != 1:
        raise ValueError(f'Need one unambiguous canonical source configuration for {year}; found {len(matches)}')
    return matches[0]
...
def extract_year(year, geometry, root=ROOT, execution_code=None):
    ...
    source_path = source_configuration_path(root, year)
    ...
    if record_path.exists():
        ...
        if old.get('input_key') == input_key:
            return old
        # A new extraction request with changed inputs computes a new checkpoint;
        # it cannot silently relabel the old numerical arrays as the new run.
        print(f'Global atlas NPP {year}: inputs changed; recomputing in a new frozen cache', flush=True)
    cache = root / 'NPPExtraction/data/work/global_atlas_reference'
...
def main(argv=None):
    ...
    if args.extract:
        ...
        source_copy = ROOT / OUTPUT / 'execution_sources' / f"global_npp_reference_{execution_code['tools/global_npp_reference.py']}.py"
