...
word bounding boxes and reconstructs columns from x-coordinates, so a
misassignment shows up as a visibly ragged grid. PyMuPDF is preferred for
cross-platform use; Poppler remains available as a fallback.
...
from pdf_backend import choose_backend, word_boxes
...
def bbox_xml(pdf: str, page: int) -> str:
    """Return Poppler `-bbox-layout` XHTML for a single page.

    This compatibility helper always requires Poppler. Normal extraction through
    `get_words` selects PyMuPDF automatically when it is available.
    """
    out = subprocess.run(
...
def get_words(
    ...
    if selected == "pymupdf":
        words = [Word(text, x0, y0, x1, y1) for x0, y0, x1, y1, text in word_boxes(pdf, page, selected)]
    else:
...
def main() -> None:
    ...
    lines = get_words(args.pdf, args.page, merge_gap=args.merge_gap, backend=args.backend)
