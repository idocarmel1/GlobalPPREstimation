...
Apply the paper's documented member placement (including accepted synonyms) first.
The filters below constrain inference when that direct evidence does not decide
the assignment; they do not override the model's own placement. Check every
candidate of a composite against the evidence, even if the validator accepts it.
