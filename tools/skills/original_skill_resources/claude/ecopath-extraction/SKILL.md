...
| `Biomass_accumulation.csv` | BA per group, absolute and/or as a rate — blank where no numeric BA was reported |
...
`Taxonomy.xlsx` is outside this standalone extraction skill. Use the combined
`ecopath-paper-to-ppr` skill when group membership is also requested.
The database JSON reads one if a directory happens to
...
`scripts/`, `references/` and `assets/` below are relative to that root, including
when this workflow is read from `references/` in a generated skill. Invoke scripts
with quoted absolute paths and keep outputs in the selected workspace, never in
the installed skill directory. Run `scripts/check_environment.py` before the
first extraction. Use the available Python interpreter and active shell syntax;
the command examples show arguments, not a requirement to use Bash.

## Step 0: is the source actually a model?

Read `references/source-bundle.md` to inventory the paper, supplements,
spreadsheets and prior calculations, assigning sources to each distinct model.
...
word bounding boxes. For rotated landscape pages, use `scripts/rotate_pdf.py`
to make a working copy and render it to check orientation before parsing.
...
```bash
python "<skill-root>/scripts/prose_sweep.py" paper.pdf --family pointers   # "see text", "voir texte", ...
python "<skill-root>/scripts/prose_sweep.py" paper.pdf --numbers-only      # the full sweep
```
...
```bash
python "<skill-root>/scripts/prose_sweep.py" paper.pdf --family accumulation
```
...
```bash
python "<skill-root>/scripts/write_outputs.py" model.json --outdir <extraction-set-dir>
```
...
```bash
python "<skill-root>/scripts/write_outputs.py" "<model.json>" --outdir "<output-parent>"
python "<skill-root>/scripts/validate.py" "<model-directory-printed-by-writer>"
```
...
```bash
python "<skill-root>/scripts/validate.py" <model-dir>
```
...
```bash
python "<skill-root>/scripts/database_json.py" -d <model-dir> --update-report
```
...
The command-line entry points support `--help`. Core conversion requires
`openpyxl`, `pandas` and `numpy`. PDF extraction prefers PyMuPDF and falls back to
Poppler. `scripts/render_pdf_page.py` and `scripts/rotate_pdf.py` provide portable
visual checks and working copies. See `requirements.txt` and the environment probe.
