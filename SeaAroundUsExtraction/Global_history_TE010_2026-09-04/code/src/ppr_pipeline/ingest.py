...
def _category_totals(
    ...
    # Unstack only observed taxon-key tuples. pivot_table(dropna=False) expands
    # the Cartesian product of all four labels, exhausting memory on rich EEZs.
    pivot = grouped.set_index(keys + [category_column])["tonnes"].unstack(
        category_column, fill_value=0.0
    ).reset_index()
