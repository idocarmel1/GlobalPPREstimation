...
- `eez_output/PPR_EEZ_validation_executed.html` and the accompanying `.ipynb`: removed from version control. Both were executed against the pre-migration table schema and read `jensen_comparison.csv`, which the current pipeline no longer produces. They are regenerated by `build_eez_notebook.py` once the tables in this directory are next produced by a pipeline run under the current schema.
- `eez_output/eez_execution_record.json`: removed, via `eez_execution.invalidate_execution`. It recorded status `passed` and fingerprinted the two notebook artifacts above, which no longer exist, so it asserted a successful execution that had not happened. `build_eez_notebook.py` writes a fresh record on the next successful notebook run; until then `tools/package_eez_release.py` refuses to package, which is the intended behaviour.
