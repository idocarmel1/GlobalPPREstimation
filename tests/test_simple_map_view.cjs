...
function fixture(){
  ...
  const context={network:{units:{},simple_units:{EEZ_001:simple}},metricState:state,chosenModels:{},DB:{year:2019,annual:{2019:{}}},
    ...
    selectedId:null,colorExtent:[0,1],regionGroup:{clearLayers:()=>{}},markerGroup:{clearLayers:()=>{}},addRegion:()=>{},updateLegend:()=>{},refresh:()=>{}};
...
test('finite independent results color nonpilot regions',()=>{
  ...
  const region={unit_id:'EEZ_001',networkResult:{value:50}};
  assert.equal(app.context.pprColor(region),'#colored');assert.equal(app.context.baseRegionStyle(region).fillColor,'#colored');
});

test('pilot filter uses model membership even for missing results and restores All rank selection',()=>{
  const app=fixture();
  const select={value:'pilot',options:[{value:'all',textContent:'All 366'},{value:'pilot',textContent:'Pilot model set (10)'},{value:'25',textContent:'Top 25'}]};
  app.nodes.rankFilter=select;
  app.context.network.units.EEZ_002={models:[]};
  app.context.passRegion=r=>select.value==='all'||(r.ppr_rank!=null&&r.ppr_rank<=Number(select.value));
  app.run('const priorPassRegion=passRegion;','applyYear(DB.year);');
  assert.equal(app.context.passRegion({unit_id:'EEZ_002',ppr_rank:null}),true);
  assert.equal(select.value,'pilot');
  assert.equal(app.context.passRegion({unit_id:'EEZ_001',ppr_rank:1}),false);
  select.value='all';assert.equal(app.context.passRegion({unit_id:'HS_018',ppr_rank:null}),true);
  select.value='25';assert.equal(app.context.passRegion({unit_id:'HS_018',ppr_rank:null}),false);
});
